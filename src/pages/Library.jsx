...
function Library() {
  ...
  const renderPlaylists = () => (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 font-mono">
      {playlistImages.map((url, index) => (
        <motion.div
          ...
          className="bg-secondary rounded-lg p-4 cursor-pointer hover:bg-secondary/80 shadow-sm hover:shadow-lg font-mono"
        >
          <img
            src={url}
            alt={'Playlist Cover ${index + 1}'}
            className="w-full aspect-square object-cover rounded-md mb-4 font-mono"
          />
          <h3 className="font-medium truncate text-wheat font-mono">My Playlist #{index + 1}</h3>
